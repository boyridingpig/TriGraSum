...
class TriGraSum(BaseModel):
    """ Transformer """
    def __init__(self, config):
        super(TriGraSum, self).__init__()
        ...
        # )
        # —— 改为 LLM 获取 subtoken 嵌入 ——
        # 加载 LLM tokenizer 与 model
        self.llm_tokenizer = AutoTokenizer.from_pretrained(
            "mradermacher/Code-Summary-Llama-3.2-3B-Instruct-GGUF",
            use_fast = True
        )
        self.llm_model = AutoModel.from_pretrained(
            "mradermacher/Code-Summary-Llama-3.2-3B-Instruct-GGUF"
        )
        # 如果 LLM 的 hidden_size ≠ config.dim，需要线性投射
        llm_hidden = self.llm_model.config.hidden_size
        self.llm_stok_proj = nn.Linear(llm_hidden, config.dim)
        self.llm_tok_proj = nn.Linear(llm_hidden, config.dim)
        # mid‑granularity graph attention
        self.gat1 = GATConv(config.dim, config.dim,
                            heads=config.num_heads, concat=False)
        self.gat2 = GATConv(config.dim, config.dim,
                            heads=config.num_heads, concat=False)
    ...
    def forward(self,
                ...
                stok_seq_rep,
                tok_seq_rep,
                tok_grammar_seqs,  # 新增：原始 grammar type 列表
                stm_seq_rep,
                token_to_stm_maps,  # 新增：token->stm 映射
                stok2tok_mat,
                stok2stm_mat,
                src_map):
    ...
    def encode(self,
        ...
        stok_texts = [
            [self.stok_vocab.itos()[i] for i in seq.tolist()]
            for seq in stok_seq_rep
        ]
        enc_stok = self.llm_tokenizer(
            stok_texts,
            is_split_into_words=True,
            return_tensors="pt",
            padding=True, truncation=True
        ).to(stok_seq_rep.device)
        with torch.no_grad():
            stok_llm = self.llm_model(**enc_stok).last_hidden_state  # [B, L1, H]
        stok_seq_emb = self.llm_stok_proj(stok_llm)  # [B, L1, D]
        stok_seq_emb = stok_seq_emb[:, : stok_seq_rep.size(1), :]  # 对齐到原长

        tok_texts = [
            [self.tok_vocab.itos()[i] for i in seq.tolist()]
            for seq in tok_seq_rep
        ]
        enc_tok = self.llm_tokenizer(
            tok_texts,
            is_split_into_words=True,
            return_tensors="pt",
            padding=True, truncation=True
        ).to(tok_seq_rep.device)
        with torch.no_grad():
            tok_llm = self.llm_model(**enc_tok).last_hidden_state  # [B, L2, H]
        tok_seq_emb = self.llm_tok_proj(tok_llm)  # [B, L2, D]
        tok_seq_emb = tok_seq_emb[:, : tok_seq_rep.size(1), :]

        # — mid‑granularity graph construction + GAT —
        B, T, D = tok_seq_emb.size()
        graphs = []
        for gram_seq, t2s in zip(tok_grammar_seqs, token_to_stm_maps):
            ng = len(gram_seq)
            ns = max(t2s) + 1
            edges = []
            # grammar-node i ↔ token-node i
            for i in range(ng):
                edges += [[i, ng + i],
                          [ng + i, i]]
            # grammar-node i ↔ stm-node t2s[i]
            for i, s in enumerate(t2s):
                edges += [[i, 2 * ng + s],
                          [2 * ng + s, i]]
            ei = torch.tensor(edges, dtype=torch.long).t().contiguous()
            graphs.append(Data(edge_index=ei,
                               num_nodes=2 * ng + ns))
        batch_graph = Batch.from_data_list(graphs).to(tok_seq_emb.device)
        node_feats = tok_seq_emb.view(-1, D)
        # 第一层 GAT + 激活
        h = self.gat1(node_feats, batch_graph.edge_index)
        h = F.elu(h)
        # 第二层 GAT
        h = self.gat2(h, batch_graph.edge_index)
        # reshape、切片回 token 部分
        h = h.view(B, -1, D)
        tok_seq_emb = h[:, ng:ng + T, :]

        # — end GAT —

        stm_seq_emb = self.src_stm_embedding(stm_seq_rep)  # `[batch, stm, dim]`

        # 5) 送入内部 Hi3AttnEncoder，得到三路中间表示
        mid_stok, mid_tok, mid_stm = self.encoder(
            stok_seq_emb, tok_seq_emb, stm_seq_emb,
            stok2tok_mat, stok2stm_mat,
            stok_pad_mask.unsqueeze(1),
            tok_pad_mask.unsqueeze(1),
            stm_pad_mask.unsqueeze(1)
        )
        if return_dict:
            return {
                'stok_seq_rep': mid_stok,  # 融合后的细粒度
                'stok_pad_mask': stok_pad_mask,
            }
        return mid_stok, stok_pad_mask
