...
class TransformerDecoder(nn.Module):
    ...
    #
    #     return out
    def forward(self, tgt, memory_bank, src_pad_mask, tgt_mask, layer_cache=None, return_attn=False):
        query = self.self_attn(tgt, tgt_mask, layer_cache)
        query_norm = self.self_attn_norm(self.dropout(query) + tgt)

        # 修改：让context_attn支持输出attn
        if return_attn:
            # 你需要让 MultiHeadedCrossAttention 支持 return_attn=True，返回(attn_output, attn_weights)
            mid, cross_attn = self.context_attn(memory_bank, query_norm, src_pad_mask, layer_cache, return_attn=True)
        else:
            mid = self.context_attn(memory_bank, query_norm, src_pad_mask, layer_cache)
            cross_attn = None
        ...
        if return_attn:
            return out, cross_attn  # 新增返回cross-attn
        else:
            return out
